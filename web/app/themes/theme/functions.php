...
if ( ! function_exists( 'upbds_setup' ) ) :
	...
	function upbds_setup() {
		 ...
		 * to change 'upbrands' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'upbrands', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'upbrands' ),
			)
		);
...
function upbds_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'upbrands' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'upbrands' ),
...
function upbds_scripts() {
	wp_enqueue_style( 'upbrands-style', get_stylesheet_uri(), array(), UPBDS_VERSION );
