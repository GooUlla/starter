...
if ( ! function_exists( 'upbds_posted_on' ) ) :
	...
	function upbds_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted on %s', 'post date', 'upbrands' ),
...
if ( ! function_exists( 'upbds_posted_by' ) ) :
	...
	function upbds_posted_by() {
		$byline = sprintf(
			...
			esc_html_x( 'by %s', 'post author', 'upbrands' ),
...
if ( ! function_exists( 'upbds_entry_footer' ) ) :
	...
	function upbds_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categories_list = get_the_category_list( esc_html__( ', ', 'upbrands' ) );
			if ( $categories_list ) {
				/* translators: 1: list of categories. */
				printf( '<span>' . esc_html__( 'Posted in %1$s', 'upbrands' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'upbrands' ) );
			if ( $tags_list ) {
				/* translators: 1: list of tags. */
				printf( '<span>' . esc_html__( 'Tagged %1$s', 'upbrands' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span> on %s</span>', 'upbrands' ),
						array(
							'span' => array(
								'class' => array(),
							),
